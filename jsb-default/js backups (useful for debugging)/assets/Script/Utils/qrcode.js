...
var t = 1;
for (t = 1; t < 40; t++) {
for (var e = QRRSBlock.getRSBlocks(t, this.errorCorrectLevel), r = new QRBitBuffer(), o = 0, i = 0; i < e.length; i++) o += e[i].dataCount;
for (i = 0; i < this.dataList.length; i++) {
var n = this.dataList[i];
r.put(n.mode, 4);
r.put(n.getLength(), QRUtil.getLengthInBits(n.mode, t));
n.write(r);
}
...
for (var r = this.errorCorrectLevel << 3 | e, o = QRUtil.getBCHTypeInfo(r), i = 0; i < 15; i++) {
var n = !t && 1 == (o >> i & 1);
...
for (var o = QRRSBlock.getRSBlocks(t, e), i = new QRBitBuffer(), n = 0; n < r.length; n++) {
var a = r[n];
i.put(a.mode, 4);
i.put(a.getLength(), QRUtil.getLengthInBits(a.mode, t));
a.write(i);
}
var s = 0;
...
for (;i.getLengthInBits() % 8 != 0; ) i.putBit(!1);
for (;!(i.getLengthInBits() >= 8 * s); ) {
i.put(QRCode.PAD0, 8);
if (i.getLengthInBits() >= 8 * s) break;
i.put(QRCode.PAD1, 8);
}
return QRCode.createBytes(i, o);
};
...
r += u;
var f = QRUtil.getErrorCorrectPolynomial(h), g = new QRPolynomial(n[s], f.getLength() - 1).mod(f);
...
for (l = 0; l < o; l++) for (s = 0; s < e.length; s++) l < n[s].length && (c[Q++] = n[s][l]);
for (l = 0; l < i; l++) for (s = 0; s < e.length; s++) l < a[s].length && (c[Q++] = a[s][l]);
return c;
};
...
for (var n = 0, a = t.isDark(o, i), s = -1; s <= 1; s++) if (!(o + s < 0 || e <= o + s)) for (var u = -1; u <= 1; u++) i + u < 0 || e <= i + u || 0 == s && 0 == u || a == t.isDark(o + s, i + u) && n++;
n > 5 && (r += 3 + n - 5);
}
...
for (var o = r.length / 3, i = new Array(), n = 0; n < o; n++) for (var a = r[3 * n + 0], s = r[3 * n + 1], u = r[3 * n + 2], h = 0; h < a; h++) i.push(new QRRSBlock(s, u));
return i;
};
